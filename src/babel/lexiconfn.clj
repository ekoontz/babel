...
(defn unify [ & args]
  ...
  (let [retval (apply unifyc args)]
    (if (not (fail? retval))
      retval
      (let [arg1-sem (log/error (str "arg1's sem: " (strip-refs (get-in (first args) [:synsem :sem]))))
            arg2-sem (log/error (str "arg2's sem: " (strip-refs (get-in (second args) [:synsem :sem]))))
            arg1-cat (log/error (str "arg1's subcat: " (strip-refs (get-in (first args) [:synsem :subcat]))))
            arg2-cat (log/error (str "arg2's subcat: " (strip-refs (get-in (second args) [:synsem :subcat]))))
            arg1-sem (log/error (str "arg1's cat: " (strip-refs (get-in (first args) [:synsem :cat]))))
            arg2-sem (log/error (str "arg2's cat: " (strip-refs (get-in (second args) [:synsem :cat]))))
            fail-path (log/error (str "fail-path: " (get-fail-path (first args) (second args))))
            fail-path (log/error (str "arg1: " (dissoc (strip-refs (first args)) :serialized)))
            fail-path (log/error (str "arg2: " (dissoc (strip-refs (second args)) :serialized)))
            message (str "Failed to unify lexemes"
                         (if (= 2 (.size args))
                           (str 
                                "; arg1's sem: " (strip-refs (get-in (first args) [:synsem :sem]))
                                "; arg2's sem: " (strip-refs (get-in (second args) [:synsem :sem]))
                                "; arg1's cat: " (strip-refs (get-in (first args) [:synsem :cat]))
                                "; arg2's cat: " (strip-refs (get-in (second args) [:synsem :cat]))
                                "; arg1's lin: " (strip-refs (get-in (first args) [:lineage]))
                                "; arg2's lin: " (strip-refs (get-in (second args) [:lineage]))
                                "; arg1's subcat: " (strip-refs (get-in (first args) [:synsem :subcat]))
                                "; arg2's subcat: " (strip-refs (get-in (second args) [:synsem :subcat]))
                                ": failed path: " (get-fail-path (first args)
                                                                 (second args))
                                "; arg1's value: " (get-in (first args)
                                                   (get-fail-path (first args)
                                                                  (second args)))
                                "; arg2's value: " (get-in (second args)
                                                   (get-fail-path (first args)
                                                                  (second args))))
                           ""))]
